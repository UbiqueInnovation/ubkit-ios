...
extension Bundle {
    ...
    convenience init?(locale: Locale, in bundle: Bundle = .main) {
        ...
        guard let languageCode = localeComponents[NSLocale.Key.languageCode.rawValue] else {
            Localization.logger.error("No language code found in locale \(locale.identifier)", accessLevel: .public)
            return nil
        }
        ...
        if let bundleFromIndentifier = bundle.path(forResource: bundleIdentifier, ofType: typeOfFile) {
            ...
            Localization.logger.debug("Loading bundle from language key for locale \(locale.identifier).")
            // Load the bundle using only the language code
            bundlePath = bundleFromLanguageCode
        } else {
            Localization.logger.error("No bundle found in \(bundle.bundlePath) for \(locale.identifier)", accessLevel: .public)
