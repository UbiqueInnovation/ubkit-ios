...
// MARK: - UBSwiftFormatPlugin

/// A Swift Package Manager `CommandPlugin` and `XcodeCommandPlugin` that executes
/// `UBSwiftFormatTool` to format source code in Swift package targets according
/// to the UB Swift Style Guide.
@main
struct UBSwiftFormatPlugin {

    /// Calls the `UBSwiftFormatTool` executable with the given arguments
    func performCommand(
    ...
    {
        let process = Process()
        process.launchPath = try context.tool(named: "UBSwiftFormatTool").path.string
...
extension UBSwiftFormatPlugin: CommandPlugin {
...
#if canImport(XcodeProjectPlugin)
extension UBSwiftFormatPlugin: XcodeCommandPlugin {
